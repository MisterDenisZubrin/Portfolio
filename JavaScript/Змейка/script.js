...
function startGame() {
    ...
    problemTimer = setInterval(createProblem, PROBLEM_SPEED);
}
...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && new_unit !== undefined && clearPath(new_unit)) {
...
function createProblem() {
    ...
    while (!problemCreated) {
        var problemX = Math.floor(Math.random() * FIELD_SIZE_X);
        var problemY = Math.floor(Math.random() * FIELD_SIZE_Y);
        var problemCell = document.getElementsByClassName('cell-' + problemY + '-' + problemX)[0];
        var problemCellClasses = problemCell.getAttribute('class').split(' ');

        // Проверка на змейку
        if (!problemCellClasses.includes('snake-unit') && !problemCellClasses.includes('food')) {
            var classes = '';
            for (var i = 0; i < problemCellClasses.length; i++) {
                classes += problemCellClasses[i] + ' ';
            }
            problemCell.setAttribute('class', classes + 'problem-unit');
            problemCreated = true;
        }
    }
}
/**
 * Функция проверяет, свободна ли дорога для змейки
 * @param unit  
 * @returns {boolean}
 */
function clearPath(unit) {
    var check = false;
    var unit_classes = unit.getAttribute('class').split(' ');
